...
For backdoor attacks, we also usually calculate the \textit{Clean Accuracy Drop} (CAD), which is the amount of percentage points the accuracy of the clean model has decreased after it was backdoored.
% However, the performance of the ASR model is not measured using accuracy, but with WER.
% That is why we will measure the decrease in performance for the ASR model using the \textit{Clean Word Error Rate Increase} (CWERI).
...
\section{Experiment}
% Specify the specific settings and individual attacks you performed.
  % Backdoor attack (JingleBack): poisoning rate, ...
  % Evasion attack (FGSM and PGD): ...

% Specify the transliteration and speaker you want the models to pick when they get the backdoor.
